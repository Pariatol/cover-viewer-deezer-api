...
function App() {
  ...
  let listRap = ['Eminem','Snoop Dogg','Dr Dre','Nas','Wu-Tang Clan','Jay-Z','Tupac','Notorious Big','Migos','Offset','Quavo','21 Savage','Young Thug','Rich The Kid','Future','Gucci Mane','Travis Scott','Lil Uzi Vert','Kodak Black','Rae Sremmurd','Kid Cudi','Mobb Deep','Lords of the Underground','Run DMC','Childish Gambino','A$AP Rocky','Kendrick Lamar','Kanye West','XXXTentacion','Tyler, The Creator','MAC MILLER','J. Cole','Metro Boomin',
  'Public Enemy','A Tribe Called Quest','Ice Cube','Ol\' Dirty Bastard','De La Soul','Method Man','KRS-One','The Pharcyde','Mos Def','The Roots','Redman','MF DOOM','Ghostface Killah','Earl Sweatshirt','Death Grips','Wiz Khalifa','The Weeknd','G-Eazy','The Game','Nipsey Hussle'];
...
<nav class="navbar navbar-expand-md navbar-dark fixed-top" id="main-nav">
        <div class="container">
            ...
            <div class="collapse navbar-collapse" id="navbarCollapse">
                <ul class="navbar-nav ml-auto d-flex flex-column font-weight-bold">
                    ...
                    <li class="nav-item mr-1">
                      <a class="nav-link text-light" onClick={(e)=>handleClick(e,'rap')}>Rap</a>
                    </li>
                    {fav.length>0?
                    <li class="nav-item mr-1">
      ...
      {genre==='rap'?<Cover genre={listRap} band={band} album={album} url={url} isLoading={isLoading} setFav={setFav} fav={fav} setUrl={setUrl} setAlbum={setAlbum} setBand={setBand} setIsLoading={setIsLoading}/>:null}
...
const Cover = (props) => {
  ...
  return (
    ...
    <div className="cover">
      ...
      <div className="album">
        {props.album} 
      <div className="buttons"></div> <i class="far fa-plus-square" title='Add This Album To Favorites' onClick={()=>addToFav(props.fav,props.url,props.band,props.album)}></i> <a href={youtubeSearch} target="_blank"><i class="fab fa-youtube" title='Search This Album On Youtube'></i></a> <a href={amazonSearch} target="_blank"><i class="fab fa-amazon" title='Buy This Album On Amazon'></i></a></div>
      <div className="new">
    ...
    <div className="favPage">
      ...
      {props.fav.map(item=>{
        return(
          ...
          <div className="album">
            {item.album}<div className="buttons"></div> <i class="fas fa-minus-square"  title='Remove This Album From Favorites' onClick={()=>removeFromFav(props.fav, item.id)}></i> <a href={`https://www.youtube.com/results?search_query=${item.band}+${item.album}+full+album`} target="_blank"><i class="fab fa-youtube"  title='Search This Album On Youtube'></i></a> <a href={`https://www.amazon.fr/s?k=${item.band}+${item.album}&__mk_fr_FR=ÅMÅŽÕÑ&ref=nb_sb_noss_1`} target="_blank"><i class="fab fa-amazon" title='Search This Album On Amazon'></i></a></div>
            </div>     
    );
  })}
  </div>)
